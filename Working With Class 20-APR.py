...
class Scrapper(Url, ArticleUrlConfig, ImgConfig, PublishConfig, HeadlineConfig, TextConfig):
    def Scrape_news(self):
        merged_data = ""
        url_details_lst = Url().get_domain_url()  # Get all domain url table details
        for url_id in range(len(url_details_lst)):  # get all url one by one
            article_text_config_details = TextConfig().get_article_text_config(url_details_lst[url_id][0])  # get all article text config details
            article_url_extracted_lst = Url().get_article_url(url_details_lst[url_id][0],url_details_lst[url_id][1])  # get all articles url in lst
            article_topic_head_lst = HeadlineConfig().get_article_topic_head_config(url_details_lst[url_id][0])  # get news headline config details
            article_pub_date_lst = PublishConfig().get_article_publish_date_config(url_details_lst[url_id][0])  # get publish date config details
            article_img_config_lst = ImgConfig().get_article_img_config(url_details_lst[url_id][0])  # get news image config details
